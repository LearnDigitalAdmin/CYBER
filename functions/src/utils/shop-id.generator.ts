...
function isEasyPattern(id: number): boolean {
  ...
  // Pattern 2: Sequential ascending (0123, 1234, 2345, etc.)
  ...
  // Pattern 3: Alternating pairs (1122, 2233, 1212, 2121, etc.)
  if (/^(.)\1(.)\2$/.test(str) || /^(.)(.)\1\2$/.test(str)) {
    return true;
  }

  // Pattern 4: Mirror patterns (1221, 1331, 2332, etc.)
  if (str === str.split('').reverse().join('')) {
    return true;
  }

  // Pattern 5: Very low numbers
  if (id < 1000) {
    return true;
  }
...
export async function generateNextShopId(): Promise<string> {
  ...
  try {
    const result = await db.runTransaction(async (transaction) => {
      ...
      while (!validIdFound && searchAttempts < 1000) {
