...
import { X, Upload, Crop, FileText, Download, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
...
interface UploadedImage {
  ...
  status: 'pending' | 'uploaded' | 'cropped' | 'processing' | 'complete';
}
...
const ImageToPdfModal: React.FC<ImageToPdfModalProps> = ({ user, isOpen, onClose }) => {
  ...
  const handleProceedToCrop = useCallback(() => {
    ...
    // Start cropping from first image
  ...
  const handleCropComplete = useCallback((cropData: { x: number; y: number; width: number; height: number }) => {
    ...
    // Move to next image or proceed to config
    ...
    try {
      ...
      const result = await convertImagesToPdf(
        ...
        (progress: ConversionProgress) => {
          setProcessingProgress(Math.round(progress.progress));
  ...
  return (
    <>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                  >
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start">
                  <input
                    type="checkbox"
                    id="enhancements"
                    checked={config.enableEnhancements}
                    onChange={(e) => setConfig({ ...config, enableEnhancements: e.target.checked })}
                    className="mt-1 mr-2"
                  />
                  <label htmlFor="enhancements" className="text-sm text-gray-700">
                    <span className="font-medium">Enable image enhancements</span>
                    <p className="text-gray-500 mt-1">Auto-rotate, normalize contrast, and sharpen images for better quality</p>
                  </label>
                </div>
      ...
      {/* Crop Modal */}
      {currentStep === 'crop' && currentCropIndex !== null && images[currentCropIndex] && (
        <ImageCropComponent
