...
import { 
  ...
  FileText, Download, Trash2, Search, Plus, ArrowUpRight,
...
const plotYanguData = {
  assets: [
    { id: 1, name: 'John Kamau', type: 'Landlord', phone: '0712345678', properties: 2, tenants: 15, revenue: 125000, status: 'active' },
    { id: 2, name: 'Sunrise Properties Ltd', type: 'Agent', phone: '0723456789', properties: 8, tenants: 45, revenue: 450000, status: 'active' },
    { id: 3, name: 'Mary Wanjiku', type: 'Landlord', phone: '0734567890', properties: 1, tenants: 8, revenue: 64000, status: 'active' },
    { id: 4, name: 'Nairobi Homes Agency', type: 'Agent', phone: '0745678901', properties: 12, tenants: 67, revenue: 670000, status: 'pending' }
  ],
  monthlyTrends: [
    { month: 'Apr', income: 45000, assets: 8 },
    { month: 'May', income: 67000, assets: 12 },
    { month: 'Jun', income: 89000, assets: 18 },
    { month: 'Jul', income: 112000, assets: 25 },
    { month: 'Aug', income: 145000, assets: 32 },
    { month: 'Sep', income: 178000, assets: 38 }
  ],
  stats: {
    totalAssets: 38,
    totalTenants: 135,
...
const PlotTab = ({ onOpenTerminal }: { onOpenTerminal: (asset: any) => void }) => {
  ...
  const filteredAssets = plotYanguData.assets.filter(asset => {
    const matchesSearch = asset.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         asset.phone.includes(searchTerm);
    const matchesFilter = filterType === 'all' || asset.type.toLowerCase() === filterType;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          label="Total Assets"
          value={plotYanguData.stats.totalAssets}
          trend="+12%"
          color="cyan"
        />
        <StatCard
          ...
          label="Total Tenants"
          value={plotYanguData.stats.totalTenants}
          trend="+8%"
          color="emerald"
        />
        <StatCard
          ...
          value={`KES ${plotYanguData.stats.monthlyRevenue.toLocaleString()}`}
          trend="+23%"
          color="violet"
        />
        <StatCard
          ...
          value={plotYanguData.stats.pendingRenewals}
          trend="5 due"
          color="amber"
        />
      </div>
      ...
      <div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6">
        ...
        <div className="h-64 flex items-end justify-between gap-2">
          {plotYanguData.monthlyTrends.map((data, idx) => (
                ...
                <tr key={asset.id} className="border-b border-gray-800 hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="py-4 px-4">
                    <span className={`inline-flex px-3 py-1 rounded-full text-xs font-medium ${
                      asset.type === 'Agent' ? 'bg-violet-500/10 text-violet-400 border border-violet-500/30' : 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30'
                    }`}>
                      {asset.type}
                    </span>
                  </td>
                  <td className="py-4 px-4 text-sm text-gray-400">{asset.phone}</td>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
        <button className="flex items-center justify-center gap-2 px-6 py-4 bg-cyan-500 text-black rounded-xl hover:bg-cyan-400 transition-all shadow-lg shadow-cyan-500/30 font-semibold">
          <Plus className="w-5 h-5" />
          <span>Add Landlord</span>
        </button>
        <button className="flex items-center justify-center gap-2 px-6 py-4 bg-transparent border-2 border-cyan-500 text-cyan-400 hover:bg-cyan-500 hover:text-black transition-all rounded-xl font-semibold">
          <Plus className="w-5 h-5" />
          <span>Add Agent</span>
        </button>
      </div>
